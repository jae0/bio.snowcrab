...
This is a RMarkdown document designed to render a PDF document. 

To generate output, rename this file to the correct year and then
copy it and the media directory to work/report location, such as:

  ~/bio.data/bio.snowcrab/reports/  (default below)

Eg., run the following in R:

  year.assessment = 2023
  ID = "snowcrab_presentation"

  fnmd = paste( ID, "_", year.assessment, "_", "general_summary", ".rmd", sep="" )

  sourcedir = file.path( "~", "bio", "bio.snowcrab", "inst", "markdown")
  destdir = file.path( "~", "bio.data", "bio.snowcrab", "reports")

  file.copy( from=file.path(sourcedir, fnmd ), to=file.path(destdir, fnmd), overwrite=TRUE )
  # make sure media directory is copied over or symlinked too  or specified as media_loc above in YAML

  year.assessment = 2023
  ID = "snowcrab_presentation"

  media_loc = file.path( destdir, "media" ) 

  fn_rap_rmd  = file.path( destdir, fnmd )
  ...
  rmarkdown::render( fn_rap_rmd, params=pm, output_dir=destdir )
