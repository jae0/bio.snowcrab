...
This is a Markdown document ... To create HTML or PDF, etc, run: 
  ...
  make quarto FN=05.biomass_index_sizestructured_carstm YR=2023 SOURCE=~/projects/bio.snowcrab/inst/markdown WK=~/bio.data/bio.snowcrab/reports # {via Quarto}

  make rmarkdown FN=05.biomass_index_sizestructured_carstm YR=2023 SOURCE=~/projects/bio.snowcrab/inst/markdown WK=~/bio.data/bio.snowcrab/reports {via Rmarkdown}
...
for (snowcrab_filter_class in c(  "M0", "M1", "M2", "M3", "M4", "f.mat" ) ) {
  ...
  carstm_model_label= paste( "default", snowcrab_filter_class, sep="_" )
    ...
    pA = parameters_numerical_dynamics( yrs=yrs, snowcrab_filter_class=snowcrab_filter_class,  spec_bio=spec_bio, save_location=carstm_results_directory ) 
    pN = pA$pN
    pW = pA$pW
    pH = pA$pH
    pA = NULL
  ...
  if ( spatiotemporal_model ) {
    ...
    res = carstm_model( p=pN, data=M[ iq, ], sppoly=sppoly, 
      ...
      posterior_simulations_to_retain = c("predictions"),
       control.inla = list( int.strategy="eb", strategy="laplace",   restart=1 , h=0.01 ),   # 
  ...
  # some maps and plots
    ...
    for (vns in c( "number", "meansize", "habitat") ) {
      ...
      # maps
      res = carstm_model( p=p, DS="carstm_randomeffects",  sppoly = sppoly ) # to load currently saved results
     ...
     res = carstm_model( p=p, DS="carstm_predictions",  sppoly = sppoly ) # to load currently saved results
...
#   if (grepl("size_structured", model_variation)) {
      # fishery landings has a weekly time step = 2/52 ~ 0.0385 ~ 0.04  X dt=0.01 seems to work best
        ...
        fishery_model_data_inputs( year.assessment=year.assessment, 
          ...
          for_julia=TRUE, 
          save_location=carstm_results_directory 
        )
#  }
