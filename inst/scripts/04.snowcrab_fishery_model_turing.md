...
```R
  ## just a reminder that this is from the bottom of the file 03.biomass_index_carstm.r
  ## note the output directory .. this location is used below in the Julia section
  
  if (0) {
    # already run in C, so no need to run here, unless it needs to be refreshed
