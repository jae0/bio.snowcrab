...
if ( spatiotemporal_model ) {
  ...
  mo = 10^6
  M$data_offset[io] = M$data_offset[io] * mo  # ( number / km^2 ) * mo ==> this forces everything to be expressed as  no. / m^2  .. including predictions (offset==1)
...
if (assimilate_numbers_and_size ) {
  ...
  carstm_assimilate(pN=pN, pW=pW, DS="compute" )
  ...
  outputdir = file.path( pN$modeldir, pN$carstm_model_label, "aggregated_biomass_timeseries" )
  ...
  outputdir = file.path( pN$modeldir, pN$carstm_model_label, "predicted.biomass.densitites" )
