...
The snow crab biomass index is derived from the convolution of three separate Bayesian model solutions via posterior simulation.

  - Poisson model of positive valued numbers offset by swept area in space and time 
  - Mean body size (mass) in space and time 
  - Presence-absence of organisms, where presence or absence is defined on a quantile threshold of numerical density presence_absence
...
```r
  ...
  additional_features = snowcrab_features_tmap(pN)  
...
## Part 4: prep data for integration into the fishery model (discrete version ofthe logistic model)
