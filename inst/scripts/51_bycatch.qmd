...
First set up environment. Can be made more efficient y doing intermediate saves (to do).

(note figure size needs to be adjusted directly by Quarto layout options ... to do)
...
#| fig-height: 15
  ...
  text( max(o$bct, na.rm=TRUE)*0.8, 0, labels=paste( "Snow crab CPUE (At sea obs., mean): ", o$bct_sc, " kg/trap"), col="darkred", cex=0.9 )
  # dev.off()
```
...
#| warning: false
#| error: false 
#| label: entanglements_all
#| tbl-cap: "Entanglements Maritimes"

  # get large megafauna:
  oss = o$obs  # subset for region of interst
...
#| fig-height: 6
  ...
  plot( o$spec ~ o$bct, xlab = "At sea observed catch rate in snow crab fishery (kg/trap)", ylab="Species", type="p", cex=1.1, pch=19, col="darkorange", xlim=c(0, max(o$bct, na.rm=TRUE)*1.4), yaxt="n" )  
  text( o$bct, o$spec,  labels=o$species, pos=4, srt=0 , cex=0.8, col="darkslateblue")
  text( max(o$bct, na.rm=TRUE)*0.85, 1, labels=paste( "Snow crab CPUE (At sea obs., mean): ", o$bct_sc, " kg/trap"), col="darkred", cex=0.9 )
  # dev.off()
```
...
#| tbl-cap: "Entanglements NENS"
  ...
  whales = oss[ grep("whale", common, ignore.case=TRUE), ]
  print(whales[, .(yr, lon, lat, uid)] )

  leatherback = oss[ grep("LEATHERBACK", common, ignore.case=TRUE), ]
  print(leatherback[, .(yr, lon, lat, uid)])
 
  basking_shark = oss[ grep("BASKING SHARK",  common, ignore.case=TRUE), ]
  print(basking_shark[, .(yr, lon, lat, uid)])
...
#| fig-height: 14
  ...
  plot( o$spec ~ o$bct, xlab = "At sea observed catch rate in snow crab fishery (kg/trap)", ylab="Species", type="p", cex=1.1, pch=19, col="darkorange", xlim=c(0, max(o$bct, na.rm=TRUE)*1.4), yaxt="n" )  
  text( o$bct, o$spec,  labels=o$species, pos=4, srt=0 , cex=0.8, col="darkslateblue")
  text( max(o$bct, na.rm=TRUE)*0.8, 1, labels=paste( "Snow crab CPUE (At sea obs., mean): ", o$bct_sc, " kg/trap"), col="darkred", cex=0.8 )
  # dev.off()
```
...
#| fig-height: 6
  ...
  plot( o$spec ~ o$bct, xlab = "At sea observed catch rate in snow crab fishery (kg/trap)", ylab="Species", type="p", cex=1.1, pch=19, col="darkorange", xlim=c(0, max(o$bct, na.rm=TRUE)*1.4), yaxt="n" )  
  text( o$bct, o$spec,  labels=o$species, pos=4, srt=0 , cex=0.8, col="darkslateblue")
  text( max(o$bct, na.rm=TRUE)*0.8, 1, labels=paste( "Snow crab CPUE (At sea obs., mean): ", o$bct_sc, " kg/trap"), col="darkred", cex=0.9 )
  # dev.off()
```
