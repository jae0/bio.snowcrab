...
# look at efficiency of snow crab capture vs snow crab sdiscard (soft-shelled, sublegal or female crab):
  ...
  region="cfa4x"
    ...
    whales = obs[ grep("whale", common, ignore.case=TRUE), ]
    whales[, .N, by=.(yr)] 
    # 2012, -58.04 44.97 
    
    leatherback = obs[ grep("LEATHERBACK", common, ignore.case=TRUE), ]
    leatherback[, .N, by=.(yr)]
    #     yr N
    # 1: 2001 1
    # 2: 2005 2
    ...
    basking_shark = obs[ grep("BASKING SHARK",  common, ignore.case=TRUE), ]
    basking_shark[, .N, by=.(yr)]
    #     yr N
    # 1: 2011 1 -60.01 44.7
...
Estimate via carstm: requires a Poisson model of each species of catch (number) with offset of landings and covariates
