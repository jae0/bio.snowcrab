...
@model function fishery_model_turing_ode( m0, m1, m2, kmu, ksd, tspan, prob, N=length(m0), ::Type{T}=Float64 ) where {T}  
    ...
    # observation model
    # @. m0 ~ TruncatedNormal( (ymean *q) + qc, bosd, -5.0, 5.0 )  # m0 in SD units 
    @. m0 ~ TruncatedNormal( (ymean .+ qc) * .q, bosd, -5.0, 5.0 )  # m0 in SD units 

end



@model function fishery_model_turing_incremental_ode( m0, m1, m2, kmu, ksd, tspan, prob, N=length(m0), ::Type{T}=Float64 ) where {T}  
    # biomass process model: dn/dt = r n (1-n/K) - removed ; b, removed are not normalized by K  
    # priors
...
fmod = fishery_model_turing_incremental_ode( m0, m1, m2, kmu, ksd,  tspan, prob )

fmod = fishery_model_turing_ode( m0, m1, m2, kmu, ksd,  tspan, prob )
...
plot!(survey_time, yhat  ; color=[1 2])
  ...
  fmod_pred = fmod( m0_pred, kmu, ksd,  tspan, prob  ) 
