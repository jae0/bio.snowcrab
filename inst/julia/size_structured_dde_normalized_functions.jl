...
@model function size_structured_dde_turing_testing( S, kmu, tspan, prob, nS, 
  ...
  #= note: 
     exp(0.1)-1.0 = 0.1052  .. 10% mortality (mode)
     exp(0.2)-1.0 = 0.2214  .. 20% mortality (mode)
     exp(0.3)-1.0 = 0.3499  .. 30% 
     exp(0.4)-1.0 = 0.4918  .. 40%
  =#
  d ~   filldist( β(0.1, 12), nS ) # plot(x->pdf(LogNormal(0.2, 1.0), x), xlim=(0, 2)) 
  d2 ~  filldist( β(0.5,  6), nS ) # plot(x->pdf(LogNormal(0.2, 1.0), x), xlim=(0, 2)) 

  #= note: 
    exp(0.70)-1.0 = 1.014   .. ~70% (moult) transition rate per year (mode)
    exp(0.80)-1.0 = 1.225   .. ~80% (moult) transition rate per year (mode)
    exp(0.90)-1.0 = 1.460   .. ~90% (moult) transition rate per year (mode)
    exp(0.95)-1.0 = 1.586   .. ~95% (moult) transition rate per year (mode) 
  =#
  v ~   filldist( Normal( 1.0, 0.1 ),  4 ) # transition rates # plot(x->pdf(β(0.99, 10), x), xlim=(0,1))  
...
@model function size_structured_dde_turing_north( S, kmu, tspan, prob, nS, 
  ...
  #= note: 
    ...
    log( exp(0.4)-1.0 ) = log(0.4918 ) = -0.7096 .. 40%
  =#
  d ~   filldist( LogNormal( -1.050, 0.5 ), nS ) # plot(x->pdf(LogNormal(-2.252, 0.25), x), xlim=(0, 2)) 
  d2 ~  filldist( LogNormal( -1.050, 0.5 ), nS ) # plot(x->pdf(LogNormal(-1.508, 0.25), x), xlim=(0, 2)) 

  #= note: 
    log( exp(0.80)-1.0) = log(1.226)  = 0.2038 .. ~90% (moult) transition rate per year (mode)
    log( exp(0.90)-1.0) = log(1.46)  = 0.3782  .. ~90% (moult) transition rate per year (mode)
    log( exp(0.95)-1.0) = log(1.586) = 0.461   .. ~95% (moult) transition rate per year (mode) 
  =#
  v ~   filldist( LogNormal( 0.3782, 0.25 ),  4 ) # transition rates # plot(x->pdf(LogNormal(0.2038, 0.5), x), xlim=(0,2))  

  u0 ~  filldist( Beta(2, 2), nS )  # plot(x->pdf(Beta(2, 2), x), xlim=(0,1)) # uniform 
...
@model function size_structured_dde_turing_south( S, kmu, tspan, prob, nS, 
  ...
  #= note: 
    ...
    log( exp(0.2)-1.0 ) = log(0.2214 ) = -1.508  .. 20% mortality (mode)
    log( exp(0.3)-1.0 ) = log(0.3499 ) = -1.050  .. 30% 
    log( exp(5)-1.0 ) = log(0.4918 ) = -0.7096 .. 40%
  =#
  d ~   filldist( LogNormal( -1.050, 0.5 ), nS ) # plot(x->pdf(LogNormal(-2.252, 0.25), x), xlim=(0, 2)) 
  d2 ~  filldist( LogNormal( -1.050, 0.5 ), nS ) # plot(x->pdf(LogNormal(-1.508, 0.25), x), xlim=(0, 2)) 
