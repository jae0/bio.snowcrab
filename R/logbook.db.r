  ...
  # -------
    ...
    if (DS %in% c("logbook.filtered.positions", "logbook.filtered.positions.redo")) {
      ...
      z = aegis_lookup( parameters="bathymetry", LOCS=lgbk[, c("lon", "lat")], project_class="core", output_format="points" , DS="aggregated_data", space_resolution=p$pres, variable_name="z.mean"  ) # core=="rawdata"
  ...
  # -----------------------------
    ...
    if (DS %in% c("fisheries.complete", "fisheries.complete.redo" )) {
      ...
      if (length(ii)>0) {
        logbook$z[ii] = aegis_lookup( parameters="bathymetry", LOCS=logbook[ ii, c("lon", "lat")],  project_class="core", output_format="points" , DS="aggregated_data", space_resolution=p$pres, variable_name="z.mean"  ) # core=="rawdata"
      }
      ...
      if (length(ii)>0){
        logbook$t[ii] = aegis_lookup( parameters="temperature", LOCS=logbook[ ii, c("lon", "lat", "timestamp")], project_class="core", output_format="points", DS="aggregated_data", space_resolution=p$pres, time_resolution=p$tres, year.assessment=p$year.assessment, variable_name="t.mean", tz="wAmerica/Halifax"  )
      }
