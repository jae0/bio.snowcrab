...
CA.insertTAC = function(cfa23.tac, cfa24.tac, Millbrook.tac = 250, nens.tac, xxxx.tac, xxxxW.tac, year){
  CA.proj.path = file.path(project.datadirectory("bio.snowcrab","data", "CA","CA_db"))
  ...
  statement = paste("SELECT * FROM TAC where area = '4XW' AND yr = ",year, "", sep = "")
  rs = ROracle::dbGetQuery(con, statement )
  
  if(nrow(rs) == 0){
    statement = paste("INSERT INTO TAC(YR, AREA, TAC) VALUES('", year,"', '","4XW","', '", xxxxW.tac,"')", sep = "")
    rs = ROracle::dbSendQuery(con, statement)
  }   
  ROracle::dbDisconnect(con)
