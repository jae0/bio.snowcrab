...
#     sb = bio.snowcrab::fishery_model( DS="data_aggregated_timeseries", p=p  )
  ...
  if (DS=="logistic_parameters") {
    ...
    # priors
    if (!exists("Kmu", out$standata)) out$standata$Kmu =  c( 6.0, 60.0, 2.0 )   ## based upon prior historical analyses
    ...
    if (!exists("qsd", out$standata)) out$standata$qsd =  c( 0.2, 0.2, 0.2 ) * out$standata$qmu   
